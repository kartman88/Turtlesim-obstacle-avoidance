...
def load_obstacle(obstacle):
    lista = []
    for cono in obstacle:
        elem = (cono.x, cono.y, 1)
        lista.append(elem)
    return lista

def stampa_path(percorso):
    for waypoint in percorso:
...
def main():
    ...
    #track = rospy.wait_for_message("/coni", point_array, timeout=None)
    #obstacle_list = load_obstacle (track.cones)
    ...
    print("INSERIRE IL GOAL (X)")
    x = float(input())
    print("INSERIRE IL GOAL (Y)")
    y = float(input())
    ...
    if path is None:
        ...
        percorso_convertito = converti_lista(path)
